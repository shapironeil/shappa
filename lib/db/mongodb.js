...
class MongoDBHelper {
    ...
    /**
     * Trova un documento
     * Ritorna null se MongoDB non è disponibile (graceful degradation)
     */
    async findOne(collectionName, query) {
        try {
            ...
            // Se MongoDB non è disponibile, ritorna null invece di lanciare errore
            if (error.message.includes('MongoDB not configured') || error.message.includes('connection failed')) {
                console.warn('⚠️ MongoDB not available for findOne, returning null');
                return null;
            }
    ...
    async upsertOne(collectionName, query, document) {
        try {
            ...
            // Se MongoDB non è disponibile, logga warning ma non bloccare
            if (error.message.includes('MongoDB not configured') || error.message.includes('connection failed')) {
                console.warn('⚠️ MongoDB not available for upsertOne, operation skipped');
                return null;
            }
            throw error; // Rilancia altri errori
        }
